...
server.tool(
  "authenticate",
  "Authenticate with ARIA Access API",
  {
    clientId: z.string().describe("ARIA client ID"),
    clientSecret: z.string().describe("ARIA client secret"),
    username: z.string().describe("ARIA username"),
...
server.tool(
  "get-resources",
  "Get available resources (machines, rooms, staff)",
  {
    resourceId: z.string().optional().describe("Resource ID"),
    resourceType: z.string().optional().describe("Resource type"),
    startDate: z.string().optional().describe("Start date (YYYY-MM-DD)"),
  ...
  async ({ resourceId, resourceType, startDate, endDate }: {
    ...
    try {
      ...
      const resources = await makeAriaRequest<any[]>("/resources", params);
...
server.tool(
  ...
  "Get patient diagnosis information",
  {
    patientId: z.string().describe("Patient ID"),
    diagnosisId: z.string().optional().describe("Diagnosis ID"),
  },
...
server.tool(
  ...
  async ({ patientId, startDate, endDate, resourceId }: {
    ...
    try {
      const params: Record<string, string> = { patientId, startDate, endDate };
      if (resourceId) params.resourceId = resourceId;

      const appointments = await makeAriaRequest<any[]>("/appointments", params);
...
server.tool(
  ...
  async ({ patientId, startDate, endDate }: {
    ...
    try {
      ...
      const billingData = await makeAriaRequest<any[]>("/billing", params);
